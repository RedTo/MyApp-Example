...
class _ContactState extends State<ContactPage>
    with AfterLayoutMixin<ContactPage> { //add the possibility to do something after first layout build
  ...
  Widget _buildItem(BuildContext context, Contact contact) {
    return Dismissible( //allows us to swipe a list element to the left or the right
      key: Key(contact.id.toString()), //identifier for a widget
      onDismissed: (direction) { //you can check which swiping direction was used through direction

        //delete the swiped contact from the database
        _contactProvider.delete(contact.id);

        setState(() { //call set state to refresh the list and trigger a rebuild
          _contacts.remove(contact); //remove the deleted contact from the "view"
        });
      ...
      child: ListTile( //the normal list tile / item
  ...
  Future<void> _getData([bool update = false]) async {
    ...
    if (update) { //if update is marked
      await _contactProvider.updateContacts(context); //fetch data from the api and update database
    }
    contacts = await _contactProvider.loadContacts(context); //load contacts from the database
